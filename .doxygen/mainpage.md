@mainpage LootLocker Unreal SDK — API Reference
...
This reference covers every public method and type in the C++ SDK.
...
Add the plugin to your project's `Plugins/` folder or install from the
[Unreal Marketplace](https://www.unrealengine.com/marketplace/).
Enable the **LootLockerSDK** plugin in **Edit → Plugins**.
...
All SDK calls are **static methods on `ULootLockerSDKManager`** and return results
asynchronously via a delegate callback whose first argument derives from
@ref FLootLockerResponse.
...
Every callback receives a response struct deriving from @ref FLootLockerResponse :
...
The SDK can maintain **independent sessions for multiple players** on the same
device (local co-op or couch multiplayer). Each call accepts an optional
`PlayerUlid` parameter. Use @ref MultiUserManagement to enumerate cached
players and switch the default player.
...
- [GitHub repository](https://github.com/lootlocker/unreal-sdk)
